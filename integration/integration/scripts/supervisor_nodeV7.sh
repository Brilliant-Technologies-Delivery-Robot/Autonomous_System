...
# Variable to store the current log file path
LOG_FILE_PATH=""
...
get_latest_log_dir() {
    # Use 'ls -t' to get the most recent directory
...
clear_costmaps_periodically() {
    while true; do
        echo "Clearing costmaps using bash file..."
        rosservice call /move_base/clear_costmaps "{}"
        sleep 10  # Adjust the interval as needed (e.g., every 30 seconds)
    done
}
